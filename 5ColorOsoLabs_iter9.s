...
DELAY       = $1388      ; Delay for color change (5000 in decimal)
...
NEXT_COLOR:
        ...
        LDX X              ; Load X into the accumulator (fixed)
        ADC X              ; Add the color offset to X (fixed)
        TAX                ; Transfer the result back to X (fixed)
